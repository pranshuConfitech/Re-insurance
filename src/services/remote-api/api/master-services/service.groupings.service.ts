...
export class ServiceGroupingsService {
    readonly QUERY_CONTEXT = `/master-data-service/v1/servicegroups`;


    getAllServiceGroupings(
    ...
    getServiceGroupingsById(
        ...
        return http
            .get<Page<CleintType>>(`${this.QUERY_CONTEXT}/${id}`)
            .pipe(map((response) => response.data));
    }
}