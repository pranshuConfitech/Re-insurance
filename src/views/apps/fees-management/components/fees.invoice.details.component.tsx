...
export default function InvoiceDetails(props: any) {
  ...
  const callAPiFunc = (field: any, prop: any, resultarr: any[], addrrList: any[]) => {
    addrrList.forEach((pr: any, i: number) => {
  ...
  const changeCommision = (e: any, i: any) => {
  ...
  const changeCategoryData = (e: any, i: any) => {
    const { name, value } = e.target
  ...
  const changePremiumAmount = (e: any, i: any) => {
    const { name, value } = e.target
    const list: any = formik.values.categorydata
  ...
  return (
    <form onSubmit={formik.handleSubmit}>
          ...
          <Grid container spacing={3} style={{ marginBottom: '20px' }}>
            ...
            <Grid item xs={6}>
              {/* <FormControl component="fieldset">
                                <RadioGroup row aria-label="position" name="invoiceType" value={formik.values.invoiceType}
                                    onChange={formik.handleChange} defaultValue="top" >
                                    <FormControlLabel
            ...
            <Grid item xs={2}>
              {query2.get('mode') !== 'view' && (
                <Button variant='contained' color='primary' onClick={handleopenClientModal}>
                  Search Client
                </Button>
              )}
      ...
      <Paper elevation={0}>
        <Box p={3} my={2}>
          <Accordion expanded={expanded} onChange={expandAccordian}>
            <AccordionSummary
