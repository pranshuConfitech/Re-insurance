...
export const extractRulesFromBenefitForPayload = (benefit: any) => {
  const rules = (benefit.ruleList || []).filter(
    // (r: any) => typeof r.internalId === 'string' && /[a-zA-Z]/.test(r.internalId)
    (r: any) => typeof r.internalId === 'string'
  );

  if (benefit.child) {
    const childRules = benefit.child
      .map((c: any) => extractRulesFromBenefitForPayload(c))
      .flat(1);
...
export const extractRulesForPayloadFromBenefitStructures = (benefitStructures: any) => {
  return benefitStructures
    .map((bs: any) => bs)
    .map(extractRulesFromBenefitForPayload)
    .flat(1);
};
