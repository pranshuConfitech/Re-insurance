...
const ModernButtonGroup = ({ value, onChange, classes }: any) => {
  const labels = ['Overview', 'Quotations', 'Invoices', 'Fund Invoices']
  ...
  return (
    <div className={classes.buttonGroup}>
      {labels.map((label, idx) => (
        <Button
          key={label}
          className={`${classes.modernButton} ${value === idx ? classes.buttonActive : classes.buttonInactive}`}
...
const InvoiceDashboardMain = () => {
  ...
  return (
    <div className={classes.root}>
      <div className={classes.titleSection}>Invoice Management</div>
      <Paper className={classes.paperContainer} elevation={0}>
        <div className={classes.headerContainer}>
          <ModernButtonGroup value={tabValue} onChange={handleTabChange} classes={classes} />
        </div>
...
export default InvoiceDashboardMain
