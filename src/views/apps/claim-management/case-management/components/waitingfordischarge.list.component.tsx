...
import DischargeUpdationModal from '../modal/waiting-for-discahrge/discharge.updation.modal'
...
const PatientListCompoent = () => {
  ...
  const dataSource3$: any = (
    ...
    pageRequest['hospitalizationStatus'] = 'WAITING_FOR_DISCHARGE'
  ...
  const admissionUpdationModalSubmit = (values: any) => {
    ...
    payload?.dischargeCertificate && reader.readAsDataURL(payload?.dischargeCertificate)
    payload?.feedbackForm && readers.readAsDataURL(payload?.feedbackForm)
  }

  return (
    <div style={{ backgroundColor: 'var(--surface-f)' }}>
      <Toast ref={toast} />
      <Typography style={{ fontSize: '16px', padding: '16px' }}>Waiting for Discharge {status[id]}</Typography>

      <DischargeUpdationModal
        data={benefitsWithCost}
