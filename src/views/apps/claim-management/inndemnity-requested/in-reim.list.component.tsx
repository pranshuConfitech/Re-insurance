...
export default function IndemnityReimClaimListComponent(props: any) {
  ...
  const dataSource$ = (
    pageRequest: any = {
      ...
      active: true,
      source: "NONE",
      claimSubStatus: "PENDING_INDEMNITY_APPROVAL"
    }
  ) => {
    ...
    return preAuthService.getRequestedIndemnnity(pageRequest).pipe(
      tap(data => {
      ...
      map(data => {
        ...
        const records = content.map((item: any) => {
          ...
          item['status'] = REIM_STATUS_MSG_MAP[item.subStatus as keyof typeof REIM_STATUS_MSG_MAP]
  ...
  const openEditSection = (preAuth: any) => {
    ...
    history.push(`/claims/claims-to-be-processed/${preAuth?.id}?mode=in_requested`)
  }
