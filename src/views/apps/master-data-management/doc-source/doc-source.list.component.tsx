...
import { DocSourceService, IndustryService, SectorsService } from '@/services/remote-api/fettle-remote-api'
...
let docSourceService$ = docSourceService.getAllDocSource()
...
const dataSource$: any = (
  ...
  return (docSourceService$ = docSourceService.getAllDocSource(pageRequest))
}
...
class DocSourceListComponent extends React.Component<any, any> {
  configuration: any
  ...
  handleOpen = () => {
    this.props.router.push('/masters/doc-source?mode=create')
  }

  openEditSection: any = (row: any) => {
    this.props.router.push(`/masters/doc-source/${row.id}?mode=edit`)
  }

  deleteSector: any = (row: any) => {
    docSourceService.deleteDocSource(row.id).subscribe((resp: any) => {
      this.props.toast.current.show({ severity: 'success', summary: 'Success', detail: 'Document Source deleted successfully', life: 3000 });
      this.setState({
        ...this.state,
        reloadTable: true
      })
...
export default withRouter(withStyles(useStyles)(DocSourceListComponent))
