...
import { SectorsService } from '@/services/remote-api/fettle-remote-api'
...
let sectorsService$ = sectorsService.getAllSectors()
...
const dataSource$: any = (
  ...
  return (sectorsService$ = sectorsService.getAllSectors(pageRequest))
}
...
class SectorsListComponent extends React.Component<any, any> {
  configuration: any
  ...
  handleOpen = () => {
    this.props.router.push('/masters/sector?mode=create')
  }

  openEditSection: any = (row: any) => {
    this.props.router.push(`/masters/sector/${row.id}?mode=edit`)
  }

  deleteSector: any = (row: any) => {
    sectorsService.deleteSector(row.id).subscribe((resp: any) => {
      this.props.toast.current.show({ severity: 'success', summary: 'Success', detail: 'Sector deleted successfully', life: 3000 });
      this.setState({
        ...this.state,
        reloadTable: true
      })
...
export default withRouter(withStyles(useStyles)(SectorsListComponent))
