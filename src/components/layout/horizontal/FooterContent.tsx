...
const FooterContent = () => {
  ...
  return (
    ...
    >
      <p>
        <span className='text-textSecondary'>{`©, Made with `}</span>
        <span>{`❤️`}</span>
        <span className='text-textSecondary'>{` powered by `}</span>
        <Link href='https://cognaisure.com/' target='_blank' className='text-primary uppercase'>
          Cognaisure
        </Link>
      </p>
      {!isBreakpointReached && (
